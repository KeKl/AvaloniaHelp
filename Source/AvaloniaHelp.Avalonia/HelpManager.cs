...
namespace AvaloniaHelp.Avalonia
{
    /// <summary>
    ///     The help manager.
    /// </summary>
    public sealed class HelpManager
    {
        ...
        /// </param>
        private HelpManager(IContentProvider contentProvider)
        {
            ...
            TableOfContent = new ReactiveList<Section>();
            
            if(AutomaticToCGeneration)
                Topics.Changed.Subscribe(args => TableOfContent = _defaultToCFactory(Instance));
        }
        ...
        private static ReactiveList<Section> CreateDefaultToc(HelpManager manager)
        {
            ...
            foreach(var topic in manager.Topics)
            {
                ...
                if (strings.Count() == 1)
                    manager.TableOfContent.Add(section);
                else
                {
